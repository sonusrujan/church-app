...
function App() {
  ...
  if (isPublicDonationRoute || isPublicStaticRoute) {
    const userChurch = isLoggedIn && memberDashboard?.church ? { id: memberDashboard.church.id, name: memberDashboard.church.name } : undefined;
    return (
      <>
        <Suspense fallback={<div className="auth-shell"><p>{t("common.loading")}</p></div>}>
        <Routes>
          <Route path="/donate" element={<PublicDonationPage isLoggedIn={isLoggedIn} userChurch={userChurch} />} />
        ...
        {/* Show bottom nav on donate page when logged in */}
        {isLoggedIn && <BottomNav isSuperAdmin={false} />}
      </>
    );
  }
  ...
  return (
    <AppContext.Provider value={contextValue}>
      ...
      <div className={`app-layout ${workspaceToneClass}`}>
        ...
        {/* Floating burger FAB (mobile only, CSS hides on desktop) */}
        <button
