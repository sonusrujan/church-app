...
/** Validate a refresh token. Returns user_id if valid, null otherwise. Rotates the token. */
export async function rotateRefreshToken(token: string): Promise<{ userId: string; newToken: string; expiresAt: Date } | null> {
  const tokenHash = hashToken(token);

  const { data: row } = await db
    .from("refresh_tokens")
    .select("id, user_id, expires_at, revoked, updated_at")
  ...
  // AUTH-7: Grace period for concurrent tab refreshes.
  ...
  if (row.revoked) {
    ...
    if (Date.now() - revokedAt < 30_000) {
      ...
      if (latest && new Date(latest.expires_at).getTime() > Date.now()) {
        ...
        return { userId: row.user_id, newToken, expiresAt };
      }
    }
    return null;
  }
